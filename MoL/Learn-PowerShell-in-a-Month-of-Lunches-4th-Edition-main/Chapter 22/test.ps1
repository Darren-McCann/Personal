function get-LastOn {    
    ...
    Write-Debug "Getting Security Logs"  
    try{      
        $eventsAndIDs = Get-WinEvent -LogName System -MaxEvents $MaxEvents <#-ComputerName $ComputerName#> |         
        ...
        Write-Debug "Error getting events"
    }    

    Write-Debug "Parsing Events"
    if ($eventsAndIDs){
    ...
    }Else {Write-Debug "No Events Found"}     
} 
